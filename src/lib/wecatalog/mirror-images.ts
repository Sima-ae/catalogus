...
function sanitizeExternalId(externalId: string): string {
  const safe = String(externalId ?? '')
    ...
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '')
  if (!safe) throw new Error('WeCatalog external id is required to mirror images')
  return safe
}

export function wecatalogImportMirrorRelativeDir(externalId: string): string {
  return `${MIRROR_SUBDIR}/${sanitizeExternalId(externalId)}`
}

export function isWecatalogImportMirrorPath(url: string | null | undefined): boolean {
...
async function downloadRemoteImage(url: string): Promise<{ buffer: Buffer; contentType: string | null }> {
  const res = await fetchWecatalogRemoteUrl(url, {
...
export async function mirrorWecatalogProductImages(
  externalId: string,
  imageUrls: string[]
): Promise<string[]> {
  ...
  const productDir = wecatalogImportMirrorRelativeDir(externalId)
  const remoteUrls = cleaned.filter((url) => isRemoteHttpUrl(url) && !isWecatalogImportMirrorPath(url))
...
export function wecatalogImportMirrorPathForIndex(
  ...
  return catalogImagePublicPath(
    `${wecatalogImportMirrorRelativeDir(externalId)}/${padIndex(index)}.${ext}`
  )
}
