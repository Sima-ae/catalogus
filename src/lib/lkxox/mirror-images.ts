...
function sanitizeExternalId(externalId: string): string {
  const safe = String(externalId ?? '')
    ...
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '')
  if (!safe) throw new Error('Lkxox external id is required to mirror images')
  return safe
}

export function lkxoxImportMirrorRelativeDir(externalId: string): string {
  return `${MIRROR_SUBDIR}/${sanitizeExternalId(externalId)}`
}

export function isLkxoxImportMirrorPath(url: string | null | undefined): boolean {
...
async function downloadRemoteImage(url: string): Promise<{ buffer: Buffer; contentType: string | null }> {
  const res = await fetchLkxoxRemoteUrl(url, {
...
export async function mirrorLkxoxProductImages(
  externalId: string,
  imageUrls: string[]
): Promise<string[]> {
  ...
  const productDir = lkxoxImportMirrorRelativeDir(externalId)
  const remoteUrls = cleaned.filter((url) => isRemoteHttpUrl(url) && !isLkxoxImportMirrorPath(url))
  ...
  for (const url of cleaned) {
    if (isLkxoxImportMirrorPath(url)) {
...
export function lkxoxImportMirrorPathForIndex(externalId: string, index: number, ext: string): string {
  return catalogImagePublicPath(`${lkxoxImportMirrorRelativeDir(externalId)}/${padIndex(index)}.${ext}`)
}
