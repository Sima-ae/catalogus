...
async function downloadRemoteImage(url: string): Promise<{ buffer: Buffer; contentType: string | null }> {
  const candidates = Array.from(
    new Set([maximizeFacebookImageUrl(url), url].filter(Boolean))
  )
  let lastError: Error | null = null

  for (const downloadUrl of candidates) {
    try {
      const res = await fetchFacebookRemoteUrl(downloadUrl, {
        headers: {
          Accept: 'image/*,*/*;q=0.8',
      ...
      if (!res.ok) {
        throw new Error(`Image download failed (${res.status}): ${downloadUrl}`)
      }

      const contentType = res.headers.get('content-type')
      const buffer = Buffer.from(await res.arrayBuffer())
      if (!buffer.length) {
        throw new Error(`Empty image response: ${downloadUrl}`)
      }
      if (buffer.length > MAX_BYTES) {
        throw new Error(`Image exceeds ${MAX_BYTES} bytes: ${downloadUrl}`)
      }
      ...
      if (typeBase && !typeBase.startsWith('image/') && buffer.subarray(0, 256).includes('<html')) {
        throw new Error(`Expected image, got ${typeBase}: ${downloadUrl}`)
      }

      const dims = readJpegDimensions(buffer)
      if (dims && Math.max(dims.width, dims.height) < MIN_IMAGE_DIMENSION) {
        throw new Error(
          `Image too small (${dims.width}x${dims.height}, need ≥${MIN_IMAGE_DIMENSION}px): ${downloadUrl}`
        )
      }
