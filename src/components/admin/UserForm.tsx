...
import BuyerSiteAccessCodePanel from '@/components/admin/BuyerSiteAccessCodePanel'
...
export default function UserForm({
  ...
  const assignCodeAfterCreate = async (newUserId: string, code: string) => {
    const res = await fetch(appPath(`/api/admin/users/${newUserId}/site-access-code`), {
      method: 'POST',
    ...
    const data = await res.json()
    if (!res.ok) {
      throw new Error(typeof data.error === 'string' ? data.error : 'Failed to assign access code')
    }
    return data
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isCreate) {
        const created = data as UserListRow
        const code = draftAccessCode.trim()
        if (form.role === 'buyer' && code) {
          try {
            await assignCodeAfterCreate(created.id, code)
          } catch (assignErr) {
            setError(
              assignErr instanceof Error
                ? `User created, but code assignment failed: ${assignErr.message}`
                : 'User created, but code assignment failed'
            )
            return
          }
        }
        if (onCreated) {
          onCreated({ ...created, site_access_code: code || created.site_access_code || null })
          setForm(emptyForm)
          setDraftAccessCode('')
        }
      } else {
  ...
  const showCodePanel = form.role === 'buyer'

  return (
    <>
      <div
        className={`grid gap-6 items-start ${
          showCodePanel ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1 max-w-xl'
        }`}
      >
        <form onSubmit={handleSubmit} className="card space-y-4 h-full">
        ...
        {showCodePanel && (
          <BuyerSiteAccessCodePanel
            userId={userId}
            role={form.role}
            readOnly={effectiveReadOnly}
            draftCode={draftAccessCode}
            onDraftCodeChange={setDraftAccessCode}
          />
        )}
      </div>
